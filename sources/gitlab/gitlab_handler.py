...
def fetch_variables():
    ...
    gl_config = load_gitlab_config(local_config)
    ...
    group_vars = load_group_vars(gl_config, gitlab_client)
    project_vars = load_project_vars(gl_config, gitlab_client)
...
def find_project(gitlab_client, wanted_project):
...
def get_group_names(project_path):
...
def load_group_vars(gitlab_config, gitlab_client):
    ...
    wanted_groups = get_group_names(gitlab_config[PROJECT_PATH_KEY])
    variables = {}
    path_elements = []
    for wanted_group in wanted_groups:
        path_elements.append(wanted_group)
        valid_group = find_group(gitlab_client, path_elements, wanted_group)
...
def find_group(gitlab_client, path_elements, wanted_group):
